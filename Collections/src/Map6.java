import java.util.*;
public class Map6 {
    static Hashtable<Integer, Hashtable<Integer, Integer>> graph = new Hashtable<>();
    ...
    static void dfs(int node, int cost, boolean[] visited, List<Integer> path) {
        ...
        Hashtable<Integer, Integer> neighbors = graph.get(node);
        for (int next : neighbors.keySet()) {
            int edgeCost = neighbors.get(next);
